...
// take random usernames for seeding , use chapGPT
...
func Seed(store store.Storage, db *sql.DB) {
	...
	//ex45 making changes to seed func
...
func generateUsers(num int) []*store.User {
	...
	for i := 0; i < num; i++ {
		users[i] = &store.User{
			...
			// Password: "123123",
			//created at is created at database layer so no need to use it
			//ex 56 creating RoleID, when user is creating we know RoleId will be 1. if we delete and redo also its same
			Role: store.Role{
				Name: "user",
			},
		}
